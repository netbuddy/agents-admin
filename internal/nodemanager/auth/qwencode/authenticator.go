...
// Start 启动认证流程
func (a *Authenticator) Start(ctx context.Context, task *auth.AuthTask, containerClient *auth.Client) (<-chan *auth.AuthStatus, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *Authenticator) runAuthFlow(ctx context.Context) {
	...
	if err := a.containerClient.CreateVolume(ctx, volumeName); err != nil {
	...
	containerCfg := &auth.ContainerConfig{
		Name:       containerName,
		Image:      a.task.Image,
	...
	containerID, err := a.containerClient.CreateContainer(ctx, containerCfg)
	if err != nil {
	...
	containerIO, err := a.containerClient.AttachContainer(ctx, containerID)
	if err != nil {
	...
	if err := a.containerClient.StartContainer(ctx, containerID); err != nil {
...
func (a *Authenticator) processOutput(ctx context.Context, reader io.Reader, volumeName string) {
	...
	for {
		...
		case <-authCheckTicker.C:
			...
			// 检查容器是否仍在运行
			running, err := a.containerClient.IsContainerRunning(ctx, a.containerID)
			if err != nil {
...
func (a *Authenticator) checkAuthSuccess(ctx context.Context, volumeName string) bool {
	...
	exists, err := a.containerClient.FileExistsInVolume(ctx, volumeName, a.task.AuthDir, authFile)
	if err != nil {
...
func (a *Authenticator) cleanupLocked() error {
	...
	if a.containerID != "" && a.containerClient != nil {
		ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
		defer cancel()

		// 停止并删除容器
		a.containerClient.StopContainer(ctx, a.containerID, nil)
		a.containerClient.RemoveContainer(ctx, a.containerID, true)
		a.containerID = ""
	}
