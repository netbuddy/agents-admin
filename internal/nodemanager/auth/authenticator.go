// Package auth 定义 Agent 认证器接口
//
// 提供统一的认证抽象，支持不同 Agent 的 OAuth/DeviceCode/APIKey 等认证方式
//
// 文件组织：
//   - authenticator.go: Authenticator 接口和注册表
//   - container.go: 认证流程的容器操作（Docker 客户端封装）
//   - qwencode/: Qwencode OAuth 认证实现
package auth
...
// Authenticator 认证器接口
//
// 每种 Agent 实现自己的认证器，处理特定的认证流程
type Authenticator interface {
	// AgentType 返回支持的 Agent 类型
	AgentType() string
	...
	// Start 启动认证流程
	// 返回的 channel 用于接收状态更新
	Start(ctx context.Context, task *AuthTask, containerClient *Client) (<-chan *AuthStatus, error)
