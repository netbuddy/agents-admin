...
func TestAdapter_Name(t *testing.T) {
	a := New()
	if a.Name() != "qwencode-v1" {
		t.Errorf("Name() = %v, want qwencode-v1", a.Name())
	}
}
...
func TestAdapter_Validate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			agent:   &adapter.AgentConfig{Type: "qwencode"},
			wantErr: false,
		},
		{
			name:    "valid qwen-code type",
			agent:   &adapter.AgentConfig{Type: "qwen-code"},
			wantErr: false,
		},
		{
			name:    "valid qwen type",
			agent:   &adapter.AgentConfig{Type: "qwen"},
			wantErr: false,
		},
		{
			name:    "invalid type",
...
func TestAdapter_BuildCommand(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cfg, err := a.BuildCommand(ctx, spec, tt.agent)
			if err != nil {
...
func TestAdapter_ParseEvent(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantType: adapter.EventFileWrite,
		},
		{
			name:     "done event",
			line:     `{"type":"done"}`,
...
func TestAdapter_CollectArtifacts(t *testing.T) {
	a := New()
	ctx := context.Background()

	artifacts, err := a.CollectArtifacts(ctx, "/workspace")
	if err != nil {
