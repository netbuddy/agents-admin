// Package redis AuthSession 缓存操作
package redis
...
func (s *Store) CreateAuthSession(ctx context.Context, session *cache.AuthSession) error {
	key := cache.KeyAuthSession + session.TaskID
	...
	pipe.Expire(ctx, key, cache.TTLAuthSession)

	idxKey := cache.KeyAuthSessionByAccount + session.AccountID
	pipe.Set(ctx, idxKey, session.TaskID, cache.TTLAuthSession)
	...
	log.Printf("[Redis/Cache] Created auth session: %s (account=%s, status=%s)", session.TaskID, session.AccountID, session.Status)
	return nil
}

// GetAuthSession 获取认证会话
func (s *Store) GetAuthSession(ctx context.Context, taskID string) (*cache.AuthSession, error) {
	key := cache.KeyAuthSession + taskID
...
func (s *Store) GetAuthSessionByAccountID(ctx context.Context, accountID string) (*cache.AuthSession, error) {
	idxKey := cache.KeyAuthSessionByAccount + accountID
...
func (s *Store) UpdateAuthSession(ctx context.Context, taskID string, updates map[string]interface{}) error {
	...
	log.Printf("[Redis/Cache] Updated auth session: %s, updates=%v", taskID, updates)
	return nil
}
...
func (s *Store) DeleteAuthSession(ctx context.Context, taskID string) error {
	...
	pipe.Del(ctx, key)
	if accountID != "" {
		pipe.Del(ctx, cache.KeyAuthSessionByAccount+accountID)
	}
...
func (s *Store) ListAuthSessions(ctx context.Context) ([]*cache.AuthSession, error) {
	pattern := cache.KeyAuthSession + "*"
	keys, err := s.client.Keys(ctx, pattern).Result()
	if err != nil {
		return nil, fmt.Errorf("failed to list auth session keys: %w", err)
	}

	var sessions []*cache.AuthSession
...
func (s *Store) ListAuthSessionsByNode(ctx context.Context, nodeID string) ([]*cache.AuthSession, error) {
	allSessions, err := s.ListAuthSessions(ctx)
	if err != nil {
		return nil, err
	}

	var nodeSessions []*cache.AuthSession
...
func parseAuthSession(data map[string]string) (*cache.AuthSession, error) {
	session := &cache.AuthSession{
		TaskID:        data["task_id"],
