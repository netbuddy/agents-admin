...
// 类型别名（原定义在已迁移的 nodes.go 中）
type HeartbeatRequest = openapi.HeartbeatRequest
type UpdateNodeRequest = openapi.UpdateNodeRequest
...
func TestUpdateRunRequest_Parsing(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if string(*req.Status) != tt.wantStatus {
