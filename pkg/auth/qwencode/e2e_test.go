...
func TestQwenCodeAuthenticator_E2E(t *testing.T) {
	...
	defer func() {
		authenticator.Stop()
		dockerClient.RemoveVolume(context.Background(), task.VolumeName, true)
	}()
	...
	statusChan, err := authenticator.Start(ctx, task, dockerClient)
	if err != nil {
		// 如果是镜像不存在，跳过测试
...
func TestQwenCodeAuthenticator_Stop_E2E(t *testing.T) {
	...
	defer dockerClient.RemoveVolume(context.Background(), task.VolumeName, true)

	// 启动认证
	_, err = authenticator.Start(ctx, task, dockerClient)
	if err != nil {
