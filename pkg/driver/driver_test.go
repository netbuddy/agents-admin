...
func TestRegistryRegisterAndGet(t *testing.T) {
	...
	mockDriver := &mockDriver{name: "test-driver"}
	registry.Register(mockDriver)

	got, ok := registry.Get("test-driver")
	if !ok {
		t.Fatal("Expected driver to be found")
	}

	if got.Name() != "test-driver" {
		t.Errorf("Driver name = %v, want test-driver", got.Name())
	}
}
...
func TestRegistryGetNotFound(t *testing.T) {
	...
	if ok {
		t.Error("Expected driver not to be found")
	}
}
...
func TestRegistryList(t *testing.T) {
	...
	registry.Register(&mockDriver{name: "driver-a"})
	registry.Register(&mockDriver{name: "driver-b"})
	...
	if !found["driver-a"] || !found["driver-b"] {
		t.Errorf("Expected drivers driver-a and driver-b, got %v", names)
	}
}
...
func (m *mockDriver) Validate(agent *AgentConfig) error {
	return nil
}
func (m *mockDriver) BuildCommand(ctx context.Context, spec *TaskSpec, agent *AgentConfig) (*RunConfig, error) {
	return &RunConfig{
		Command: []string{"echo"},
		Args:    []string{"test"},
	}, nil
}
func (m *mockDriver) ParseEvent(line string) (*CanonicalEvent, error) {
	return nil, nil
}
func (m *mockDriver) CollectArtifacts(ctx context.Context, workDir string) (*Artifacts, error) {
