...
| 类别 | 文件数 | 中文字符串数 | 说明 |
|------|--------|-------------|------|
| 页面组件（app/） | 12 | ~370 | 各管理页面（含 agents/login/register） |
| 通用组件（components/） | 12 | ~231 | 侧边栏、模态框、看板卡片等 |
| 工具库（lib/） | 2 | ~33 | agent-models.ts、auth.tsx |
| 布局（layout.tsx） | 1 | 1 | `<html lang="zh">` + metadata |
| 日期格式化 | 8处 | — | 硬编码 `'zh-CN'` locale |
| **合计** | **27** | **~635** | 不含代码注释和 node_modules |
...
| 5 | 迁移 Sidebar 导航文案 | ⬜ | P1 | 8 个导航项 |
| 6 | 迁移 Header 组件文案 | ⬜ | P1 | 刷新、通知等 |
| 7 | 迁移首页（看板）文案 | ⬜ | P1 | 任务状态、操作按钮 |
| 8 | 迁移 CreateTaskModal 文案 | ⬜ | P1 | 表单标签、按钮、提示 |
| 9 | 迁移 agents 页面文案 | ⬜ | P1 | 最多中文字符串（99处） |
| 10 | 迁移 accounts 页面文案 | ⬜ | P1 | 54处 |
| 11 | 迁移 monitor 页面文案 | ⬜ | P1 | 48处 |
| 12 | 迁移 instances 页面文案 | ⬜ | P1 | 56处 |
| 13 | 迁移 proxies 页面文案 | ⬜ | P1 | 39处 |
| 14 | 迁移 runners 页面文案 | ⬜ | P1 | 24处 + alert() 需替换 |
| 15 | 迁移 TaskDetailPanel + TaskDetailClient | ⬜ | P1 | 71+22处 |
| 16 | 迁移 agent-output 组件群 | ⬜ | P1 | DebugPanel/AgentOutput/FileBlock/CommandBlock 等 |
| 17 | 迁移 login/register 页面文案 | ⬜ | P1 | 8+14处 |
| 18 | 迁移 nodes/settings 页面文案 | ⬜ | P2 | 较少字符串 |
| 19 | 迁移 agent-models.ts 模型描述 | ⬜ | P2 | 30处模型描述 |
| 20 | 统一日期时间格式化（useFormatDate hook） | ⬜ | P1 | 替换硬编码 'zh-CN' |
| 21 | 更新 `<html lang>` 为动态值 | ⬜ | P1 | 根据当前语言设置 |
| 22 | 验证静态导出 + Go embed 兼容性 | ⬜ | P0 | 确保 i18n 不破坏构建 |
| 23 | E2E 测试：语言切换功能 | ⬜ | P1 | agent-browser 测试 |
